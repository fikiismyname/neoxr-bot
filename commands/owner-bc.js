...
exports.run = {
	usage: ['bc', 'bcpc', 'bcgc', 'bctag'],
	async: async (m, { conn, _func, text, command, participants }) => {
		...
		if (command == 'bc') {
			...
			for (let id of chats) {
				if (id.endsWith('g.us')) {
					let msg = conn.cMod(id, conn.prepareMessageFromContent(id, {[c.toJSON ? q.mtype : MessageType.extendedText]: c.toJSON ? c.toJSON() : { text: c || '' }}, {contextInfo: { mentionedJid: member }, quoted: {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(id ? { remoteJid: 'status@broadcast' } : {}) }, message: { "imageMessage": { "mimetype": "image/jpeg", "caption": 'B R O A D C A S T', "jpegThumbnail": await _func.buffer('./media/images/thumb.jpg')} } }}), text || q.text)
   				 await conn.relayWAMessage(msg)
					await _func.delay(2500)
	    		}
			} m.reply(`*Successfully group broadcast.*`)
